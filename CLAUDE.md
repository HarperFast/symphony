...
`ProtectionState.config: ArcSwap<ProtectionConfig>` holds *all* protection settings as one swappable snapshot — CIDR allowlist/blocklist, JA3 blocklist, ASN blocklist + MMDB reader, rate limit parameters, concurrency cap, handshake timeout, and requireSni are all inside `ProtectionConfig`. This means one atomic `store()` in `updateConfig` reaches all checks for a listener with zero hot-path cost. `check()` calls `config.load()` once per connection, which is a single lock-free pointer load. Callers identify listeners by port (`JsHotConfig.protection: [{ port, protection }]`).

### ASN blocking (maxminddb, owned memory)
`ProtectionConfig.asn_reader: Option<Arc<Reader<Vec<u8>>>>` holds a MaxMind-format MMDB loaded into owned memory via `Reader::open_readfile`. The `Reader` is read-only and can be called concurrently without locking — the entire hot path is lock-free. Loading into owned memory means both atomic-rename rotation (what host-manager does) and in-place rewrites are safe: the data is fully copied at load time, so no page can be torn by a concurrent writer. The file-watcher event is the single freshness trigger: each `updateConfig` call re-reads the file, so there is no stale-reader risk and no mtime/size cache to reason about. Multiple listeners referencing the same path within a single config-parse pass share one `Arc` via a local `HashMap` (not a Mutex-guarded static). At load time, `reader.metadata().database_type` is validated to contain "ASN"; wrong-schema DBs are rejected with a hard error. Hot-swap failure (bad path, wrong schema) retains the previous reader (last-good semantics); initial-build failure propagates as a hard error. Lookup errors on the hot path fail open with a rate-limited `eprintln`. The `asn_blocklist: HashSet<u32>` is an O(1) lookup after the mmdb trie walk. `asnBlocklist` requires `asnDatabasePath`; setting one without the other is a construction-time hard error.

### CIDR trie (ip_network_table LPM)
`ProtectionConfig.allowlist_trie` and `blocklist_trie` are `IpNetworkTable<()>` (from the `ip_network_table` crate) — a bitwise LPM trie built at config-parse time in `precompute()`. Lookups are O(prefix bits) vs. the former O(n) linear scan over `Vec<IpNetwork>`. The source Vecs (`allowlist`, `blocklist`) are preserved for reporting (`blockedIps()`). The `ip_network` crate is a direct dep (not `ipnetwork`) because `ip_network_table` requires `ip_network::IpNetwork` as its key type; conversion from `ipnetwork::IpNetwork` happens in `build_trie()`. `IpNetworkTable` does not implement `Clone` or `Debug`, so `ProtectionConfig` derives only `Default`.
