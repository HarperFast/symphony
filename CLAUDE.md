...
### ASN blocking (maxminddb + MMDB cache)
`ProtectionConfig.asn_reader: Option<Arc<Reader<Mmap>>>` holds a memory-mapped MaxMind-format MMDB. The `Reader` is read-only and can be called concurrently without locking — the entire hot path is lock-free. The reader is obtained from a process-wide `Mutex<HashMap>` cache keyed by (canonical path, mtime_nanos, size): the Mutex is only held during config construction (cold path), never during `check()`. Multiple listeners referencing the same file share one mmap. Hot-swap failure (bad path or corrupt file) retains the previous reader (last-good semantics); initial-build failure propagates as a hard error. The `asn_blocklist: HashSet<u32>` is an O(1) lookup after the mmdb trie walk.
