//! Dead-peer detection for proxied connections.
//!
//! Two mechanisms, covering the two ways a connection outlives its client (issue #45):
//!
//! * **TCP keepalive.** Without `SO_KEEPALIVE` the `net.ipv4.tcp_keepalive_*` sysctls do not
//!   apply — they govern only sockets that opted in — so a peer that disappears without a FIN is
//!   indistinguishable from an idle subscriber for the life of the process.
//! * **A bound on the half-closed state.** `copy_bidirectional` completes only when *both*
//!   directions finish. When the upstream EOFs, tokio shuts down our write half to the client —
//!   the socket enters FIN-WAIT-2 — and then waits for a FIN the client may never send. The fd
//!   stays open, so the socket is not orphaned and `tcp_fin_timeout` never applies.
...
/// Failure is logged at debug, not warn: `arm_keepalive` on the listening socket already warned
/// once if the platform rejects these options, and this runs per connection.
...
pub fn arm_accepted(stream: &tokio::net::TcpStream, cfg: &KeepaliveConfig) {
	let params = TcpKeepalive::new()
		...
		.with_interval(cfg.interval)
		.with_retries(cfg.retries);
	if let Err(e) = SockRef::from(stream).set_tcp_keepalive(&params) {
		tracing::debug!("could not set TCP keepalive on an accepted socket: {e}");
	}
}

/// The bounded half-closed state of one proxied connection.
///
/// Armed by the *upstream* half reaching EOF, which is the terminal shape: tokio has already
/// shut down our write half to the client, so nothing further can be sent to it and no response
/// exists that a deadline could truncate. A *client*-half EOF is deliberately not armed — there
/// the surviving direction carries the upstream's response, which may legitimately be quiet for
/// a long time before its first byte.
///
...
impl HalfCloseWatch {
	pub fn new(metrics: Arc<ListenerMetrics>) -> Arc<Self> {
		Arc::new(Self {
	...
	/// nothing for `window`. Before the half-close it holds a `Notify` permit rather than a
	/// timer, so an ordinary connection arms no timer at all; `notify_one` stores its permit, so
	/// an EOF observed before this future is first polled is not lost.
...
mod tests {
	...
	async fn an_unarmed_connection_never_expires() {
		let watch = HalfCloseWatch::new(Arc::new(ListenerMetrics::default()));
		// The client half reaching EOF is the request/response half-close, not the bounded state.
		let mut client = Watched::client(Chunks { remaining: 2 }, watch.clone());
	...
	async fn an_upstream_eof_expires_after_a_quiet_window() {
		let watch = HalfCloseWatch::new(Arc::new(ListenerMetrics::default()));
		let mut upstream = Watched::upstream(Chunks { remaining: 0 }, watch.clone());
	...
	async fn activity_on_the_surviving_direction_defers_expiry() {
		...
		let mut client = Watched::client(Chunks { remaining: 1 }, watch.clone());
	...
	async fn the_gauge_returns_to_zero_when_the_connection_drops() {
		...
		let watch = HalfCloseWatch::new(metrics.clone());
		let mut upstream = Watched::upstream(Chunks { remaining: 0 }, watch.clone());
		drain(&mut upstream).await;
